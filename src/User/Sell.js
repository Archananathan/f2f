...
const Sell = ({ onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:8080/api/form/submit-request', formData);
      if (response.status === 200) {
        alert('Request submitted successfully!');
        navigate('/userpro'); // Redirect after successful submission
      } else {
      ...
      navigate('/userpro'); // Redirect even in case of failure
    }
  };
